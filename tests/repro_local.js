...
async function runLocalVerification() {
  ...
  try {
    ...
    // Switch to Neon Horizon using UI project switcher
    const switchProjBtnH = await page.waitForSelector('[data-testid="header-switch-project-btn"]');
    await switchProjBtnH.click();
    await page.waitForSelector('button.project-select-card', { timeout: 5000 });
    const neonCardH = await page.waitForSelector('button.project-select-card:has-text("The Neon Horizon")');
    await neonCardH.click();
    await page.waitForSelector('.modal-backdrop', { state: 'detached', timeout: 5000 }).catch(() => {});
    await page.goto(`${LOCAL_URL}?tab=clearance`, { waitUntil: "networkidle" });
    await page.waitForTimeout(1000);
    ...
    for (const vpWidth of [320, 375, 390, 420, 768, 1280]) {
      ...
      await page.goto(`${LOCAL_URL}?tab=overview`, { waitUntil: "networkidle" });
      await page.waitForTimeout(300);
    ...
    console.log('\n=== LOCAL PLAYWRIGHT VERIFICATION AUDIT COMPLETE: ALL PASS (A–K) ===');
  } catch (err) {
