...
describe('Contract & Invariants: Ingestion Completion Barrier & Query Synchronization (T065-T071)', () => {
  ...
  it('proves replacement screenplay ingestion immediately updates all queries atomically upon HTTP success', async () => {
    ...
    // Invariant: Immediately verify that the workspace is fully synchronized without secondary mutations
    const entities = await entityRepo.getEntitiesByProject(projectId);
    expect(entities.length).toBe(7);
    expect(entities.map((e) => e.canonicalName)).toContain('Elena Vance');
    ...
    expect(snapshot!.actionsSummary.openActions).toBeGreaterThanOrEqual(2);
  });

  it('proves GET /api/projects/:id/actions and GET /api/projects/:id/snapshot are strictly side-effect free', async () => {
    const projRes = await request(app)
      .post('/api/projects')
      .send({
    ...
    for (let i = 0; i < 5; i++) {
      await request(app).get(`/api/projects/${projectId}/actions`);
      await request(app).get(`/api/projects/${projectId}/actions?status=OPEN`);
      await request(app).get(`/api/projects/${projectId}/snapshot`);
      await request(app).get(`/api/projects/${projectId}/scenes/readiness`);
    }
