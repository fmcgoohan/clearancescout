...
- **FR-026**: High-Fidelity Occurrence & Scene Count Semantics. Clearance item occurrence badges and summaries MUST distinguish occurrences from scenes (e.g., "6 occurrences across 3 scenes" for Glacier Brew). The definitions of clearance item vs occurrence vs scene MUST be unified across Registry, Dossier, Scene Summaries, Filters, Action Center, Readiness, and Binder.
- **FR-027**: Canonical Entity Blocker De-Duplication. Scene readiness summaries and project blocker metrics MUST NOT count multiple occurrences of the same unresolved canonical entity as separate clearance blockers. Unique unresolved items count as 1 blocker (e.g., "1 clearance blocker ('Glacier Brew' across N appearances)"). Readiness scoring and binder reports MUST NOT double-count canonical blockers across occurrences.
...
- **FR-044**: Occurrence Textual Appearances vs Unresolved Risk Count Semantics. Entity occurrence badges and scene readiness rationales MUST maintain strict separation between cumulative script appearances and unresolved legal clearance blockers. In the Glacier Brew baseline (6 appearances across 3 scenes), Scene 1 exhibits 1 unresolved blocker across 3 appearances; Scene 2 exhibits 1 unresolved blocker across 1 appearance; Scene 3 exhibits 1 unresolved blocker across 2 appearances. Rationale copy MUST enforce grammatical agreement ("1 clearance blocker prevents shooting Scene 2: \"Glacier Brew\" (INSUFFICIENT_EVIDENCE, appears 1 time)"). Scene 2 sluglines MUST preserve `EXT. NEIGHBORHOOD CORNER - CONTINUOUS` and `EXT • CONTINUOUS` without stripping `CONTINUOUS`.
