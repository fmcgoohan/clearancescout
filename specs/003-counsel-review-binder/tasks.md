...
- [ ] T004 [P] Update `CanonicalEntity` schema with `isOverridden` and `latestOverride` in `server/repositories/EntityRepo.ts`
- [ ] T005 [P] Update `ClearanceBinder` schema to aggregate `overridesHistory` in `server/repositories/BinderRepo.ts`
...
- [ ] T006 [P] [US1] Contract test for enhanced binder export in `tests/contract/test_binder_export_v2.test.ts`

### Implementation for User Story 1

- [ ] T007 [US1] Update `BinderExportWorkflow` to aggregate full project summary, scene breakdown, and SHA-256 signature in `server/workflows/binderExportWorkflow.ts`
- [ ] T008 [US1] Update binder REST endpoints in `server/api/binderRoutes.ts`
- [ ] T009 [P] [US1] Enhance `BinderExportModal` with `@media print` styling, page break rules, and print-to-PDF layout in `src/components/BinderExportModal.tsx`
...
- [ ] T010 [P] [US2] Contract test for counsel override endpoints in `tests/contract/test_counsel_override.test.ts`

### Implementation for User Story 2

- [ ] T011 [P] [US2] Implement override REST endpoints (`POST /api/projects/:id/entities/:entityId/override` and `GET /api/projects/:id/entities/:entityId/overrides`) in `server/api/clearanceRoutes.ts`
- [ ] T012 [US2] Add Counsel Override form, status selector, and rationale input to `src/components/CitationDrawer.tsx`
- [ ] T013 [P] [US2] Update `EntityRegistryTable.tsx` to render "Overridden by Counsel" badges with rationale tooltips
...
- [ ] T014 [P] [US3] Component test for script highlighting and badge clicks in `tests/contract/test_script_highlighter.test.ts`

### Implementation for User Story 3

- [ ] T015 [P] [US3] Implement dynamic regex tokenization and badge rendering in `src/components/ScriptViewer.tsx`
- [ ] T016 [US3] Connect script highlight badge click events to open `CitationDrawer` and focus entity in `src/pages/WorkspacePage.tsx` and `src/App.tsx`
...
- [ ] T017 [P] Implement end-to-end integration test for counsel review and binder export workflow in `tests/integration/counsel_review_workflow.test.ts`
- [ ] T018 Run quickstart validation scenarios defined in `specs/003-counsel-review-binder/quickstart.md`
- [ ] T019 Verify production build (`tsc && vite build`) and Vitest test suite (`npm test`)
...
```bash
Task: "Implement CounselOverride model and OverrideRepo in server/repositories/OverrideRepo.ts"
Task: "Update CanonicalEntity schema with isOverridden in server/repositories/EntityRepo.ts"
Task: "Update ClearanceBinder schema to aggregate overridesHistory in server/repositories/BinderRepo.ts"
```

### Parallel User Story Tasks
```bash
Developer A: "Implement override REST endpoints in server/api/clearanceRoutes.ts"
Developer B: "Implement dynamic regex tokenization and badge rendering in src/components/ScriptViewer.tsx"
Developer C: "Enhance BinderExportModal with @media print styling in src/components/BinderExportModal.tsx"
```
...
3. **STOP and VALIDATE**: Verify binder export and download functionality.

### Incremental Feature Rollout
1. **MVP**: Downloadable Auditable Legal Clearance Binder (US1).
2. **Increment 2**: Studio Legal Counsel Decision Override with Audit Logging (US2).
3. **Increment 3**: Multi-Scene In-Script Visual Highlighter (US3).
4. **Increment 4**: End-to-End Polish & Verification (Phase 6).
