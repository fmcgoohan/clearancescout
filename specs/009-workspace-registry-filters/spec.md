...
### Session 2026-08-18
- Q: Where does filter processing occur and how are multiple filter criteria combined? → A: Filters are client-side and combine status, category, and scene with AND.
- Q: What is the initial/default filter selection for each dimension? → A: Default is All for every dimension.
- Q: What is displayed when active filters yield zero matching items? → A: Empty results show an empty state and a Clear Filters action.
- Q: Does applying filters modify stored entities, scene occurrences, or binder exports? → A: Filtering never mutates stored data or binder contents.
- Q: How is the entity count indicator formatted in the UI? → A: Visible count shows matching of total (e.g., "X of Y Entities Displayed").
...
1. **Given** the entity registry table, **When** initialized, **Then** all dimensions default to `ALL` (Status: ALL, Category: ALL, Scene: ALL) and all entities are visible.
2. **Given** the entity registry table, **When** the user selects a Clearance Status filter (e.g., `ACTION_REQUIRED`), **Then** only entities matching that status are displayed.
3. **Given** the entity registry table, **When** the user selects an Entity Category filter (e.g., `BRAND`), **Then** only entities matching that category are displayed.
4. **Given** the entity registry table, **When** the user selects a Scene filter (e.g., `Scene 1`), **Then** only entities that occur in that scene are displayed.
5. **Given** multiple active filters (e.g., `BRAND` + `REVIEW_RECOMMENDED` + `Scene 2`), **When** applied, **Then** only entities matching all active filter dimensions are displayed (logical AND).

---

### User Story 2 - Empty Filter State & Quick Reset (Priority: P2)

As a studio clearance coordinator, when my active filter combination produces zero matching entities, I want to see a clear empty state explaining which filters are active and a one-click "Clear Filters" action so that I never get stuck with an empty table.
...
**Independent Test**: Apply a filter combination with 0 matches (e.g., `ART_MUSIC` + `NO_ISSUE_SURFACED` in `Scene 3`), verify that the descriptive empty state appears with the active filter parameters, click "Clear Filters", and verify that all filters reset to `ALL` and all entities reappear.
...
**Why this priority**: Legal and clearance integrity requires that filtering is purely a client-side view presentation transformation that never mutates server state or truncates exported binders.
...
- **FR-001**: The system MUST perform multi-dimension filtering entirely on the client side without issuing network requests.
- **FR-002**: The system MUST support filtering by Clearance Status (`ALL`, `NO_ISSUE_SURFACED`, `REVIEW_RECOMMENDED`, `ACTION_REQUIRED`, `INSUFFICIENT_EVIDENCE`).
- **FR-003**: The system MUST support filtering by Entity Category (`ALL`, `BRAND`, `ART_MUSIC`, `PUBLIC_FIGURE`, `PROPRIETARY_LOCATION`, `GRAPHIC_PROP`).
- **FR-004**: The system MUST support filtering by Scene (`ALL SCENES` or specific scenes extracted from the screenplay).
- **FR-005**: The system MUST default to `ALL` across all filter dimensions upon initialization or script load.
- **FR-006**: When multiple filters are active, the system MUST compute the intersection (logical AND) across all selected filter dimensions.
- **FR-007**: The system MUST display an informative count badge showing matching entities of total entities (e.g. `X of Y Entities Displayed`).
- **FR-008**: When no entities match the selected filter combination, the system MUST display a clear empty state naming the active filter parameters and providing a "Clear Filters" action.
- **FR-009**: Filtering MUST be strictly non-destructive and read-only, executing entirely on client state without mutating stored entities, occurrences, assessments, or counsel overrides.
- **FR-010**: Exporting the clearance binder MUST always include all project entities in full regardless of active client-side filter selections.
