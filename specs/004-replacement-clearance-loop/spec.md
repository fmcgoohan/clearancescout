...
- Q: How should candidate clearance evaluations returning `INSUFFICIENT_EVIDENCE` be handled? → A: `INSUFFICIENT_EVIDENCE` is treated as a rejection. The loop continues generating a new candidate if attempts remain ($< 3$), and escalates to studio legal counsel if it occurs on the final attempt ($= 3$).
- Q: How must search API errors or outages be handled during candidate verification in `CLOUD_MODE`? → A: In `CLOUD_MODE`, any research outage or API failure must fail visibly as `INSUFFICIENT_EVIDENCE` and must NEVER silently fall back to synthetic/demo fixtures or assume candidate safety.
- Q: How should the system present replacement results after 3 failed attempts? → A: The system terminates the loop and presents the last candidate (attempt 3) alongside all accumulated collision evidence and citations, with no invented ranking score or synthetic metrics.
- Q: What exact timeline event types are authorized for the self-clearance loop? → A: Exactly and only four discrete SSE timeline event types: `REPLACEMENT_ATTEMPT`, `REPLACEMENT_RESEARCH_STARTED`, `REPLACEMENT_REJECTED`, and `REPLACEMENT_ACCEPTED`.
...
2. **Given** candidate 1 research results surface an existing registered trademark, commercial brand conflict, or insufficient evidence (status `ACTION_REQUIRED`, `REVIEW_RECOMMENDED`, or `INSUFFICIENT_EVIDENCE`), **When** candidate 1 is evaluated, **Then** the system automatically records a rejection with the specific collision/evidence rationale and triggers generation of candidate 2 with negative prompt constraints avoiding the conflicted term.
...
Production teams and studio counsel must have full visibility into the autonomous clearance loop rather than observing a black-box spinner. Every candidate generation attempt, research start, rejection event with specific collision reasoning, and final acceptance verdict must be streamed to the Observable Action Timeline via Server-Sent Events (SSE) using strictly defined event types (`REPLACEMENT_ATTEMPT`, `REPLACEMENT_RESEARCH_STARTED`, `REPLACEMENT_REJECTED`, `REPLACEMENT_ACCEPTED`).
...
**Independent Test**: Monitor the action timeline drawer during a multi-attempt replacement generation loop; verify that events for `REPLACEMENT_ATTEMPT`, `REPLACEMENT_RESEARCH_STARTED`, `REPLACEMENT_REJECTED`, and `REPLACEMENT_ACCEPTED` appear in chronological order with human-readable diagnostic metadata.
...
2. **Given** research begins for candidate 1, **When** search query executes, **Then** an action timeline event `REPLACEMENT_RESEARCH_STARTED` is emitted detailing `candidateName` and `attemptNumber: 1`.
3. **Given** candidate 1 is rejected due to a trademark collision or insufficient evidence, **When** rejection occurs, **Then** an action timeline event `REPLACEMENT_REJECTED` is emitted detailing `rejectedName`, `collisionReason`, `rejectionStatus`, and `attemptNumber: 1`.
4. **Given** candidate 2 clears research evaluation, **When** accepted, **Then** an action timeline event `REPLACEMENT_ACCEPTED` is emitted detailing `acceptedName`, `attemptNumber: 2`, `totalAttempts: 2`, and `clearanceStatus: 'NO_ISSUE_SURFACED'`.

---

### User Story 3 - Deterministic Bounding & Counsel Escalation Fallback (Priority: P3)

In scenarios where multiple candidate names in a crowded commercial product category fail clearance research, the autonomous generation loop must be strictly bounded to a deterministic maximum attempt limit (default: 3 attempts). If the maximum attempt limit is reached without obtaining a `NO_ISSUE_SURFACED` verdict, the system must terminate the loop gracefully, present the last generated candidate (attempt 3) alongside all accumulated research findings and citations (with no invented ranking score), and flag the card for manual studio legal review (`REVIEW_RECOMMENDED` or `INSUFFICIENT_EVIDENCE`), rather than hallucinating clearance or looping infinitely.
...
**Independent Test**: Simulate an entity category where all candidates return trademark collisions; verify that the loop halts exactly after 3 attempts, records all 3 rejection/attempt logs, and presents the last candidate with clear escalation notice to counsel.
...
2. **Given** a loop reaching the maximum attempt limit without full clearance, **When** presenting the card, **Then** the UI displays the last candidate (attempt 3) with an amber alert banner indicating maximum attempts were reached and presents all collision citations without any synthetic ranking score.

---

### Edge Cases

- **Search API Outage / Network Failure in `CLOUD_MODE`**: If the research tool encounters a network failure, API outage, or missing credentials in `CLOUD_MODE`, the system MUST fail visibly as `INSUFFICIENT_EVIDENCE` and must NEVER silently fall back to synthetic/demo fixtures or assume candidate safety.
...
- **FR-003**: System MUST reject any candidate whose clearance research returns `ACTION_REQUIRED`, `REVIEW_RECOMMENDED`, or `INSUFFICIENT_EVIDENCE`, capturing the specific collision/evidence reason and conflicting trademark/brand name.
...
- **FR-006**: System MUST terminate the loop and escalate to studio legal counsel if the maximum attempt limit is reached without a clean clearance verdict, presenting the last candidate (attempt 3) with status `REVIEW_RECOMMENDED` or `INSUFFICIENT_EVIDENCE` and all collision evidence without any invented ranking score.
- **FR-007**: System MUST record and emit discrete observable action timeline events via SSE strictly using these four event types: `REPLACEMENT_ATTEMPT`, `REPLACEMENT_RESEARCH_STARTED`, `REPLACEMENT_REJECTED`, and `REPLACEMENT_ACCEPTED`.
- **FR-008**: System MUST attach verified research citations and actual result provenance (`PARALLEL_LIVE`, `DEMO_FIXTURE`, `FALLBACK_FIXTURE`) to the accepted or escalated replacement card data structure.
- **FR-009**: In `CLOUD_MODE`, any search API outage or error MUST fail visibly as `INSUFFICIENT_EVIDENCE` and MUST NEVER silently use fallback fixtures or fabricate evidence.
...
- **ReplacementAttemptRecord**: Represents an individual attempt in the self-clearance loop, capturing `attemptNumber`, `candidateName`, `clearanceStatus` (`NO_ISSUE_SURFACED` | `REVIEW_RECOMMENDED` | `ACTION_REQUIRED` | `INSUFFICIENT_EVIDENCE`), `collisionRationale`, `citations`, and `timestamp`.
- **SelfClearanceResult**: Represents the final result of the self-clearance loop, containing `status` (`ACCEPTED` | `ESCALATED_TO_COUNSEL`), `totalAttempts`, `selectedCard` (the accepted card or final attempt 3 card), `attemptHistory`, and `provenanceSummary`.
...
- **SC-003**: 100% of candidate generation attempts, research starts, rejections, and acceptances emit strictly one of the 4 defined timeline events (`REPLACEMENT_ATTEMPT`, `REPLACEMENT_RESEARCH_STARTED`, `REPLACEMENT_REJECTED`, `REPLACEMENT_ACCEPTED`) received by the frontend within 200ms of event emission.
