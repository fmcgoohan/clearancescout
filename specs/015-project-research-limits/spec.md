...
### Session 2026-08-18
- Q: What is the default live quota cap and scope? → A: Default live quota is 25 Parallel plus Gemini calls per project (per project, not global).
- Q: Do offline fixtures in TEST_MODE and DEMO_MODE consume quota? → A: TEST_MODE and DEMO_MODE fixtures never consume quota.
- Q: What happens when the live quota limit is reached? → A: Exhausted quota returns 429 and a visible banner.
- Q: How should remaining quota be displayed in the UI? → A: Header shows remaining of total (e.g. `⚡ Live Quota: 25 / 25`).
- Q: Is quota tracking global or scoped per project? → A: Quota is per project, not global.
...
As a studio production administrator or clearance coordinator in `CLOUD_MODE`, I want live Parallel Search and Gemini model API calls to be tracked per project against a 25-call default quota limit, with a real-time remaining quota badge displayed in the workspace header showing remaining of total (`⚡ Live Quota: 25 / 25`), so that I can monitor API consumption and budget predictability.

**Why this priority**: Core quota protection requirement ensuring studio projects do not exceed live research allocations.

**Independent Test**: Execute live research calls in `CLOUD_MODE`; verify that the remaining quota counter decrements accurately and updates the UI header indicator.

**Acceptance Scenarios**:
1. **Given** an active project in `CLOUD_MODE` initialized with a default quota of 25 calls, **When** live Parallel Search or Gemini API calls are made, **Then** the project's consumed quota increments and the remaining quota indicator reflects the updated balance (e.g. `⚡ Live Quota: 24 / 25`).
2. **Given** the workspace header is displayed, **When** rendered on desktop or mobile, **Then** an explicit high-contrast badge displays the remaining of total quota.

---

### User Story 2 - Fail-Visible Rejection on Quota Exhaustion (Priority: P2)

As a studio legal counsel or clearance coordinator, I want research and replacement generation requests to fail visibly with an explicit 429 error and UI banner when a project's live quota is exhausted, rather than silently succeeding or returning ungrounded mocks.
...
**Independent Test**: Exhaust a project's 25-call live quota limit; trigger a subsequent clearance evaluation or replacement generation in `CLOUD_MODE`; verify the API returns a `429 Too Many Requests` error with an explicit message and the UI displays a fail-visible banner.

**Acceptance Scenarios**:
1. **Given** a project with 0 remaining live quota in `CLOUD_MODE`, **When** a clearance evaluation, retry, or replacement generation is requested, **Then** the backend rejects the request with HTTP status `429` (`"Live research quota exceeded for this project (0/25 remaining)"`).
...
- **FR-001**: The system MUST track live Parallel Search and Gemini API call consumption on a per-project basis (not global).
- **FR-002**: Each project MUST have a default live quota cap of 25 Parallel plus Gemini calls (`liveQuotaLimit = 25`).
- **FR-003**: The workspace UI MUST display an explicit, accessible remaining-of-total quota indicator in the header (e.g. `⚡ Live Quota: 25 / 25`).
...
- **FR-005**: Quota exhaustion rejections MUST fail visibly with an explicit error message (`"Live research quota exceeded for this project"`) and a visible UI error banner, and MUST NOT silently continue or fake live results.
- **FR-006**: Evaluations and replacement generations executed in `TEST_MODE` or `DEMO_MODE` using local record-replay fixtures MUST NEVER consume live project quota.
...
- **SC-002**: 100% of requests exceeding the 25-call live quota limit are rejected with HTTP 429 and fail visibly in the UI.
- **SC-003**: The remaining-of-total quota indicator (`⚡ Live Quota: X / 25`) is visible in the workspace header across desktop and mobile viewports.
...
- Default project live quota cap is 25 live calls per project, configurable via server environment variables (`PROJECT_LIVE_QUOTA_LIMIT`).
- Quota usage is persisted in `ProjectRepo` and returned with project metadata (`GET /api/projects/:id`).
